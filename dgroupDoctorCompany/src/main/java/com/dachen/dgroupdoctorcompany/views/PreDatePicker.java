 ...
 * @date 2016/7/1
 */
public class PreDatePicker extends WheelPicker {
    /**
     * 年月日
     */
     ...
     */
    public PreDatePicker(Activity activity) {
     ...
     * @see #MONTH_DAY
     */
    public PreDatePicker(Activity activity, @Mode int mode) {
        super(activity);
        this.mode = mode;
        ...
        if (mode != MONTH_DAY) {
            ...
            yearView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
                ...
                public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                    ...
                    int selectedYear = stringToYearMonthDay(item);

                    int maxDays = DateUtils.calculateDaysInMonth(selectedYear, stringToYearMonthDay(months.get(selectedMonthIndex)));
                    if (maxDays != days.size()) {
                        days.clear();
                    ...
                    if (selectedYear < currentYear) {//选择了今年之前的年，则月日随意选择
                        if (months.size() != 12 || selectedMonthIndex == 11) {//months.size() != 12，表示上一次选择的是今年
                            months.clear();
                            for (int i = 1; i <= 12; i++) {
                                months.add(DateUtils.fillZero(i));
                            }
                    ...
                    } else if (selectedYear == currentYear) {//选择今年，则只能选择今年已过的月日
                        months.clear();
        ...
        monthView.setOnWheelViewListener(new WheelView.OnWheelViewListener() {
            ...
            public void onSelected(boolean isUserScroll, int selectedIndex, String item) {
                ...
                if (mode != YEAR_MONTH) {
                    Calendar calendar = Calendar.getInstance();
                    int currentYear = calendar.get(Calendar.YEAR);
                    int selectedYear = stringToYearMonthDay(years.get(selectedYearIndex));
                    int selectedMonth = stringToYearMonthDay(item);
                    int currentMonth = calendar.get(Calendar.MONTH) + 1;
